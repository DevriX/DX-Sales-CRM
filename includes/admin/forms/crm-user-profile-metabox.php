...
<div class="crm-user-meta-details-wrp">
	<table class="form-table">
		<tbody>
			<input type="hidden" name="current_user_role" id="current_user_role" value="<?php echo esc_attr( $current_user_role[0] ); ?>">
			<tr class="dxcrm-customer-row tr <?php echo esc_attr( $crm_customer_css ); ?>">
				<th><label for="_crm_user_group_id"><?php esc_html_e( 'Select Customer' , 'dxcrm' );?></label></th>
				<td>
					<select name="_crm_user_group_id" id="_crm_user_group_id" class="crm-customer-select">
					<?php echo wp_kses_post( $customers_list ); ?>
					</select>
				</td>
			</tr>
		</tbody>
